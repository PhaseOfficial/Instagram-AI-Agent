...
export class IgClient {
    ...
    async init() {
        logger.info(`Initializing IgClient for user: ${this.username}`);
    ...
    async interactWithPosts() {
        ...
        while (postIndex <= maxPosts) {
            ...
            try {
                ...
                // Get Post URL
                const postUrl = await page.evaluate((i) => {
                    const article = document.querySelector(`article:nth-of-type(${i})`);
                    if (!article) return null;
                    const links = Array.from(article.querySelectorAll('a[href*="/p/"], a[href*="/reels/"], a[href*="/reel/"]'));
                    for (const link of links) {
                        const href = link.getAttribute('href');
                        if (href && (href.includes('/p/') || href.includes('/reels/') || href.includes('/reel/'))) {
                ...
                if (postUrl) {
                    console.log(`Navigating to post permalink: ${postUrl}`);
                    const mainPage = page;
                    await mainPage.goto(postUrl, { waitUntil: 'networkidle2' });
                    await delay(2000);

                    // Comment on the post in permalink page
                    let commentBox = null;
                    ...
                    for (const selector of permalinkSelectors) {
                        commentBox = await mainPage.$(selector);
                        if (commentBox) {
                            console.log(`Found comment box on permalink with selector: ${selector}`);
                            break;
                        }
                    }

                    if (commentBox && !suspendCommenting) {
                        console.log(`Commenting on post ${postIndex} via permalink...`);
                        ...
                        try {
                            ...
                            if (typeof result === 'string' && (result.includes("Error") || result.includes("unavailable"))) {
                            ...
                            if (comment) {
                                ...
                                if (!shouldSkipComment(comment, filterCfg)) {
                                    ...
                                    console.log(`Comment submitted for post ${postIndex}.`);
                                    if (dailyLimit > 0) await incrementIgDailyCount(1);
                                    summary.comments++;
                                    await delay(3000);
                                }
                            }
                            ...
                            commentFailures++;
                            logger.error(`Failed to generate or post comment for post ${postIndex}: ${commentError.message}`);
                            console.log(`Comment failure count: ${commentFailures}/2`);
                            if (commentFailures >= 2) {
                                suspendCommenting = true;
                                logger.warn("⚠️ Commenting SUSPENDED for this run due to 2 failures.");
                            }
                        }
                    } else if (suspendCommenting) {
                        console.log(`Commenting is suspended, skipping post ${postIndex}.`);
                    } else {
                        console.log(`Comment box NOT FOUND on permalink for post ${postIndex}.`);
                    }
                    
                    // Go back to feed
                    console.log("Returning to feed...");
                    await mainPage.goto("https://www.instagram.com/", { waitUntil: 'networkidle2' });
                    await delay(2000);
                    // Scroll back to where we were
                    await mainPage.evaluate((idx) => {
                        const articles = document.querySelectorAll('article');
                        if (articles[idx-1]) articles[idx-1].scrollIntoView();
                    }, postIndex);
                    await delay(1000);
                } else {
                    console.log(`Could not determine post URL for post ${postIndex}, skipping comment.`);
                }
